...
impl MevProtectedBroadcaster {
    ...
    pub async fn run_updater<T: BlockListProvider>(
        block_list_provider: T,
    ) -> tokio::task::JoinHandle<()> {
        loop {
            match block_list_provider.fetch_blocked_leaders().await {
                Ok(blocked_leaders) => {
            ...
            tokio::time::sleep(Self::REFRESH_LIST_DURATION).await;
        }
    }
}
