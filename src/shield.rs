...
impl YellowstoneShieldProvider {
    pub fn new(key: Pubkey, rpc: Arc<RpcClient>) -> Self {
        Self { key, rpc }
    }
    pub async fn get_blocked_ips(&self) -> anyhow::Result<Vec<SocketAddr>> {
...
pub mod test {
    ...
    pub async fn test_get_identities_for_key() {
        let rpc_provider = Arc::new(RpcClient::new("http://rpc:8899".to_string()));
        let key =
